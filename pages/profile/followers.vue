<template>
  <view class="followers-container">
    <uni-nav-bar left-icon="back" @clickLeft="goBack" title="粉丝列表" />
    ...
    <view v-else-if="isRetryVisible" class="retry-container">
      ...
      <button class="retry-button" @click="loadFollowersList">重试</button>
    </view>
    
    <!-- Empty state -->
    <view v-else-if="followersList.length === 0" class="empty-container">
      <image src="/static/images/empty-followers.png" class="empty-image"></image>
      <text class="empty-text">暂无粉丝</text>
    </view>
    
    <!-- Followers list -->
    <view v-else class="followers-list">
      <view class="user-list" v-if="!isLoading && followersList.length > 0">
        <view class="user-item" v-for="(user, index) in followersList" :key="index" @click="viewUserProfile(user)">
  ...
  setup() {
    ...
    const loadFollowersList = async () => {
      ...
      try {
        ...
        const result = await api.user.getFollowers(userId);
        console.log('Followers list:', result);
        followersList.value = result;
        
        // 标记用户关注状态
        const currentUserInfo = await userStore.fetchUserInfo();
        const userFollowing = currentUserInfo.following || [];
        
        followersList.value = followersList.value.map(user => ({
          ...user,
          isFollowing: userFollowing.some(followingId => 
            followingId === user._id || 
            (followingId._id && followingId._id === user._id)
          )
        }));
      } catch (error) {
